...
const query = async () => {
  ...
  try {
    ...
    if (response.data?.success !== import.meta.env.VITE_SUCCESS_FLAG) {
      toast.warning(
        escapeQifuHtmlMsg(response.data?.message || "查詢 API Client 失敗。"),
      );
      setConfigTotal(store.gridConfig, 0);
      return;
    }
    ...
    toast.error(
      escapeQifuHtmlMsg(error instanceof Error ? error.message : "查詢失敗。"),
    );
  } finally {
    hideLoading();
  }
...
<template>
  <Toolbar
    ...
    description="管理外部系統 API Client、Scope、配額及 Key。"
    createFlag="Y"
    refreshFlag="Y"
  ...
  <div v-show="show" class="card mb-3">
    <div class="card-body row g-2">
      ...
      <div class="col-md-2">
        <input
          v-model="store.queryParam.clientCodeLike"
          class="form-control"
      ...
      <div class="col-md-2">
        <input
          v-model="store.queryParam.clientNameLike"
          class="form-control"
          placeholder="Client 名稱"
        />
      </div>
      <div class="col-md-2">
        <select v-model="store.queryParam.systemType" class="form-select">
          <option value="">全部系統類型</option>
