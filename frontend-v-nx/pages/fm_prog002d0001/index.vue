...
import { useFmProg002d0001Store } from "./QueryPageStore";

definePageMeta({ middleware: ["auth"] });

const router = useRouter();
const store = useFmProg002d0001Store();
const rows = ref<any[]>([]);
...
store.gridConfig = getGridConfig(
  ...
  [
    ...
    { label: "員工編號", field: "employeeNo" },
    { label: "帳號", field: "account" },
    { label: "姓名", field: "displayName" },
    ...
    const response = await getAxiosInstance().post(
      import.meta.env.VITE_API_URL + PageConstants.eventNamespace + "/findPage",
      {
        field: {
          ...
          employeeNoLike: store.queryParam.employeeNo,
          accountLike: store.queryParam.account,
