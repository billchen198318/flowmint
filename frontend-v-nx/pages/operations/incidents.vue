...
import GroovyIncidentPanel from "~/components/fm/GroovyIncidentPanel.vue";
...
const openCount = computed(
  () =>
    incidents.value.filter((item: any) => item.incidentStatus === "OPEN")
      .length,
);
...
const run = async (action: "retry" | "reassign" | "terminate") => {
  ...
  if (
    action === "terminate" &&
    !window.confirm("確定要終止整個流程？此操作無法復原。")
  )
    return;
  acting.value = true;
  try {
  ...
  <main class="container-fluid py-4">
    ...
    <div class="row g-4">
      ...
      <div class="col-xl-5">
        <div
          v-if="selected"
          class="card border-0 shadow-sm sticky-xl-top incident-detail"
        >
          <div class="card-header bg-white py-3">
            <strong>Incident 明細</strong>
          </div>
          <div class="card-body">
            <dl class="row small">
              <dt class="col-4">Incident</dt>
              <dd class="col-8 text-break">{{ selected.incidentId }}</dd>
              <dt class="col-4">流程</dt>
              <dd class="col-8 text-break">{{ selected.processInstanceId }}</dd>
              <dt class="col-4">Task</dt>
              <dd class="col-8 text-break">{{ selected.taskId || "-" }}</dd>
              <dt class="col-4">錯誤</dt>
              <dd class="col-8">{{ selected.errorMessage }}</dd>
            </dl>
            <label class="form-label">解析 Context</label>
            <pre class="context-box">{{
              formattedContext(selected.contextData)
            }}</pre>
            ...
            <div v-else class="alert alert-secondary mb-0">
              {{ selected.resolvedBy || "-" }}／{{
                formatDate(selected.resolvedDate)
              }}<br />
    ...
    <GroovyIncidentPanel :tenant-id="tenantId" />
  </main>
</template>
