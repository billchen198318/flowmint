...
@Component
public class FmGroovyDesignAccess {

    private final FmTenantAccessGuard tenantAccess;

    public FmGroovyDesignAccess(FmTenantAccessGuard tenantAccess) {
    ...
    public void require(String tenantId) throws ServiceException {
        ...
        boolean explicit = user != null && user.isEnabled() && user.getRoles() != null && user.getRoles().stream()
                ...
                .anyMatch(permission -> permission != null
                        ...
                        && "FLOWMINT_GROOVY_DESIGN".equals(permission.getPermission()));
        if (!explicit || !UserUtils.isPermitted("FM_PROG004D0001U", PermissionType.CONTROLLER.name())) {
            throw new ServiceException("GROOVY_DESIGN_DENIED");
        }
